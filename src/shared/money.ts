...
export type NumberGrouping = "indian" | "international";

// The renderer may change this device-level presentation preference. Main-process exports keep
// the Indian default unless a caller opts into another grouping explicitly.
let defaultNumberGrouping: NumberGrouping = "indian";

export function setDefaultNumberGrouping(grouping: NumberGrouping): void {
  defaultNumberGrouping = grouping;
}
...
/** Format paise using Indian grouping by default, with optional international 3-digit groups. */
export function formatPaise(
  ...
  let grouped: string;
  const grouping = opts.grouping ?? defaultNumberGrouping;
  if (whole.length <= 3) {
    grouped = whole;
  } else if (grouping === "international") {
