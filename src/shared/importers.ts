...
export interface GenericJournalCsvRow {
  line: number;
  sourceId: string;
  group: string;
  date: string;
...
export function parseLedgersCsv(csvText: string): CsvParseResult<LedgerCsvRow> {
  ...
  if (idx.name === undefined)
    return {
      rows,
      errors: [
        { line: headerLineNo, message: "Missing required column: Name" },
      ],
    };
  if (idx.group === undefined)
    return {
      rows,
      errors: [
        {
          line: headerLineNo,
          message: "Missing required column: Group (or Under)",
        },
      ],
    };
  ...
  for (const { line: lineNo, cells } of records.slice(1)) {
    ...
    if (openingBalance === null) {
      errors.push({
        line: lineNo,
        message: `Invalid opening balance "${openingRaw}"`,
      });
      continue;
    }
    ...
    if (creditDaysRaw) {
      ...
      if (!Number.isFinite(n) || n < 0) {
        errors.push({
          line: lineNo,
          message: `Invalid credit days "${creditDaysRaw}"`,
        });
        continue;
      }
...
export function parseItemsCsv(csvText: string): CsvParseResult<ItemCsvRow> {
  ...
  if (idx.unit === undefined)
    return {
      rows,
      errors: [
        { line: headerLineNo, message: "Missing required column: Unit" },
      ],
    };
  ...
  for (const { line: lineNo, cells } of records.slice(1)) {
    ...
    if (gstRate === undefined) {
      errors.push({
        line: lineNo,
        message: `Invalid GST rate "${gstRateRaw}"`,
      });
      continue;
    }
    ...
    if (openingQtyMilli === null) {
      errors.push({
        line: lineNo,
        message: `Invalid opening quantity "${qtyRaw}"`,
      });
      continue;
    }
    ...
    if (valueRaw) {
      ...
      if (parsed === null) {
        errors.push({
          line: lineNo,
          message: `Invalid opening value "${valueRaw}"`,
        });
        continue;
      }
      openingValue = parsed;
      ...
      if (rateRaw) {
        ...
        if (ratePaise === null) {
          errors.push({
            line: lineNo,
            message: `Invalid opening rate "${rateRaw}"`,
          });
          continue;
        }
...
export function parseOpeningBalancesCsv(
  ...
  if (idx.ledgerName === undefined)
    return {
      rows,
      errors: [
        { line: headerLineNo, message: "Missing required column: Ledger" },
      ],
    };
  if (idx.opening === undefined)
    return {
      rows,
      errors: [
        { line: headerLineNo, message: "Missing required column: Opening" },
      ],
    };
...
// ---------- generic journal / Busy / Zoho Books / Marg transaction rows ----------
...
const JOURNAL_ALIASES: Record<string, string[]> = {
  ...
  voucherType: ["voucher type", "vch type", "type", "transaction type"],
  number: ["number", "document number", "doc no", "voucher number"],
  ledger: ["ledger", "ledger name", "account", "account name", "particulars"],
  debit: ["debit", "debit amount", "dr", "dr amount"],
  credit: ["credit", "credit amount", "cr", "cr amount"],
  narration: ["narration", "description", "notes", "memo"],
  reference: ["reference", "reference number", "ref no", "invoice number"],
  attachment: [
    "attachment",
    "attachment filename",
    "document file",
...
export function parseGenericJournalCsv(
  csvText: string,
): CsvParseResult<GenericJournalCsvRow> {
  const records = csvRecords(csvText);
  const rows: GenericJournalCsvRow[] = [];
  const errors: CsvError[] = [];
  if (!records.length)
    return { rows, errors: [{ line: 1, message: "Empty file" }] };
  const idx = buildHeaderIndex(records[0]!.cells, JOURNAL_ALIASES);
  ...
  if (idx.debit === undefined && idx.credit === undefined)
    return {
      rows,
      errors: [
        {
          line: records[0]!.line,
          message: "Missing required columns: Debit / Credit",
        },
      ],
    };
  for (const { line, cells } of records.slice(1)) {
    const group = cellAt(cells, idx.group);
    const ledger = cellAt(cells, idx.ledger);
    const rawDate = cellAt(cells, idx.date);
    const date = parseImportDate(rawDate);
    const debit = parsePlainRupees(cellAt(cells, idx.debit));
    const credit = parsePlainRupees(cellAt(cells, idx.credit));
    if (!group) {
      errors.push({ line, message: "Missing voucher group" });
      continue;
    }
    if (!ledger) {
      errors.push({ line, message: "Missing ledger" });
      continue;
    }
    if (!date) {
    ...
    if (debit === null || credit === null || (debit === 0) === (credit === 0)) {
      errors.push({
        line,
        message: "Enter exactly one positive Debit or Credit amount",
      });
      continue;
    }
    rows.push({
      line,
      sourceId: cellAt(cells, idx.sourceId) || `line-${line}`,
      group,
      date,
      voucherType: cellAt(cells, idx.voucherType) || "Journal",
      number: cellAt(cells, idx.number) || null,
      ledger,
      debit,
      credit,
      narration: cellAt(cells, idx.narration) || null,
      reference: cellAt(cells, idx.reference) || null,
      attachment: cellAt(cells, idx.attachment) || null,
    });
  }
  return { rows, errors };
}
...
export const GENERIC_JOURNAL_CSV_TEMPLATE: string[][] = [
  [
    "Source ID",
    "Voucher Group",
    "Date",
