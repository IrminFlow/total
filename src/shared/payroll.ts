...
export interface PayHeadSpec {
  ...
  value: number;
  /** False for reviewed one-off earnings such as overtime that must not be attendance-prorated again. */
  prorate?: boolean;
}
...
const nameIs = (head: PayHeadSpec, n: string): boolean =>
  head.name.trim().toLowerCase() === n;
...
export function computeMonthlyPay(
  ...
  if (e.heads && e.heads.length > 0) {
    const basicHead = e.heads.find(
      (h) => h.kind === "earning" && nameIs(h, "basic"),
    );
    ...
    for (const h of e.heads) {
      ...
      const full =
        h.calc === "flat" ? h.value : roundPaise((basicFull * h.value) / 10000);
      const amount =
        h.calc === "flat"
          ? h.prorate === false
            ? h.value
            : roundPaise(h.value * ratio)
          : roundPaise((basic * h.value) / 10000);
...
export function buildEsiCsv(rows: EsiInput[]): string {
  const header =
    "IP Number,IP Name,No of Days,Total Monthly Wages,Reason Code for Zero Workdays,Last Working Day";
  const body = rows.map((r) =>
    [
      csvCell(r.esicNo),
      csvCell(r.name),
      String(Math.round(r.payableDays)),
      String(rupees(r.gross)),
      "0",
      "",
    ].join(","),
