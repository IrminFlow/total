...
// zod 3's .safe() added an explicit ±Number.MAX_SAFE_INTEGER pair of checks; zod 4 folded that
// range into .int() itself (the "safeint" format) and left .safe() with nothing to add. The
// runtime bound is identical either way, but the published contract in
// agent-skill/voucher.schema.json is generated by walking these checks, so state the range
// rather than let it vanish from the document money callers read.
const paise = z.number().int().min(-Number.MAX_SAFE_INTEGER).max(Number.MAX_SAFE_INTEGER)
...
export const tallyImportSchema = z
  ...
  // prefault, not default: zod 4's .default() hands the value back untouched, so `{}` would
  // skip dryRun's own default. .prefault() runs `{}` through the object parse, which is what
  // zod 3's .default() did here.
  .prefault({})
...
// .prefault, not .default: see tallyImportSchema — `{}` must be parsed so the nested paise
// defaults fill in, which is zod 3's .default() behaviour.
export const gst3bManualSchema = z.object({
  itcRevRul: itcPartSchema.prefault({}),
  itcRevOth: itcPartSchema.prefault({}),
  interest: itcPartSchema.prefault({}),
  lateFee: z.object({ camt: paise.default(0), samt: paise.default(0) }).prefault({})
})
