...
/** Goods sent out for job work (roadmap E #127). */
export const jobWorkSendSchema = z.object({
  partyLedgerId: id,
  challanNo: z.string().trim().min(1).max(40),
  sentOn: isoDate,
  goodsType: z.enum(['input', 'capital']).default('input'),
  natureOfProcessing: z.string().trim().max(200).nullable().default(null),
  notes: z.string().trim().max(500).nullable().default(null),
  fromGodownId: id.nullable().default(null),
  lines: z.array(z.object({ stockItemId: id, qtyMilli: z.number().int().positive() })).min(1).max(100)
})
export type JobWorkSendInput = z.infer<typeof jobWorkSendSchema>

export const jobWorkReceiveSchema = z.object({
  challanId: id,
  receivedOn: isoDate,
  notes: z.string().trim().max(500).nullable().default(null),
  toGodownId: id.nullable().default(null),
  lines: z
    .array(
      z.object({
        stockItemId: id,
        qtyMilli: z.number().int().positive(),
        kind: z.enum(['goods', 'waste']).default('goods')
      })
    )
    .min(1)
    .max(100)
