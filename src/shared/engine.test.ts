...
describe("money", () => {
  ...
  it("supports explicit and device-default international grouping", () => {
    expect(formatPaise(1234567890, { grouping: "international" })).toBe(
      "12,345,678.90",
    );
    setDefaultNumberGrouping("international");
...
describe("dates", () => {
  ...
  it("formats display timestamps as DD-MMM-YY HH:MM", () => {
    expect(toDisplayDateTime(new Date(2026, 7, 15, 9, 5))).toBe(
      "15-Aug-26 09:05",
    );
    expect(toDisplayDateTime(new Date(2025, 0, 2, 23, 59))).toBe(
      "02-Jan-25 23:59",
    );
  });
...
describe("voucher posting rules", () => {
  ...
  const resolve = (id: number): LedgerFacts =>
    facts[id] ?? { exists: false, isCashOrBank: false };
  ...
  it("rejects unbalanced vouchers", () => {
    ...
    expect(validateVoucher(v, "journal", resolve).map((e) => e.code)).toContain(
      "unbalanced",
    );
  });
  ...
  it("enforces cash/bank rules per voucher kind", () => {
    ...
    expect(
      validateVoucher(contraBad, "contra", resolve).map((e) => e.code),
    ).toContain("cash_bank_rule");
    ...
    expect(
      validateVoucher(paymentBad, "payment", resolve).map((e) => e.code),
    ).toContain("cash_bank_rule");
  ...
  it("payment/receipt must actually have their cash/bank money side (v0.3 #65)", () => {
    ...
    expect(
      validateVoucher(paymentNoMoneySide, "payment", resolve).map(
        (e) => e.code,
      ),
    ).toContain("cash_bank_rule");
    ...
    expect(
      validateVoucher(receiptNoMoneySide, "receipt", resolve).map(
        (e) => e.code,
      ),
    ).toContain("cash_bank_rule");
  });
  ...
  it("the cash/bank lines must carry the FULL money side, unknown ledgers included (v0.3 #65)", () => {
    ...
    const codes = validateVoucher(
      paymentUnknownOnMoneySide,
      "payment",
      resolve,
    ).map((e) => e.code);
    expect(codes).toContain("unknown_ledger");
  ...
  it("requires a party ledger when billRefs are given", () => {
    const v = {
      ...
      billRefs: [
        { kind: "new" as const, name: "INV-1", amount: 5000, dueDate: null },
      ],
    };
    expect(validateVoucher(v, "journal", resolve).map((e) => e.code)).toContain(
      "bill_refs_no_party",
  ...
  it("requires billRefs to sum to the party ledger line amount", () => {
    const v = {
      ...
      billRefs: [
        { kind: "new" as const, name: "INV-1", amount: 4000, dueDate: null },
      ],
    };
    expect(validateVoucher(v, "journal", resolve).map((e) => e.code)).toContain(
      "bill_refs_mismatch",
      ...
      lines: [
        {
          ...
          drCr: "dr" as const,
          amount: 5000,
          costAllocations: [{ costCentreId: 1, amount: 6000 }],
        },
        { ledgerId: 4, drCr: "cr" as const, amount: 5000 },
      ],
    };
...
describe("GSTR-1 builder", () => {
  const b2bDoc: GstDoc = {
    ...
    invoiceValue: 118000,
    items: [
      { rate: 18, taxable: 100000, cgst: 9000, sgst: 9000, igst: 0, cess: 0 },
    ],
    hsnLines: [
      {
        hsn: "8471",
  ...
  const b2csDoc: GstDoc = {
    ...
    invoiceValue: 59000,
    items: [
      { rate: 18, taxable: 50000, cgst: 4500, sgst: 4500, igst: 0, cess: 0 },
    ],
    hsnLines: [],
  };
  ...
  it("routes documents into the right sections", () => {
    ...
    expect(json.b2b[0].inv[0].itms[0].itm_det).toEqual({
      rt: 18,
      txval: 1000,
      camt: 90,
      samt: 90,
    ...
    expect(json.b2cs[0]).toMatchObject({
      sply_ty: "INTRA",
      pos: "27",
      rt: 18,
      txval: 500,
    ...
    expect(json.hsn.hsn_b2b[0]).toMatchObject({
      hsn_sc: "8471",
      uqc: "NOS",
      qty: 2,
      txval: 1000,
  ...
  it("computes outward tax, ITC and net payable", () => {
    const docs: GstDoc[] = [
      ...
      },
      {
        ...
        invoiceValue: 11800,
        items: [
          { rate: 18, taxable: 10000, cgst: 900, sgst: 900, igst: 0, cess: 0 },
        ],
        hsnLines: [],
      },
      {
    ...
    const r = buildGstr3b(
      {
        ...
        itc: {
          impg: zero,
          isrc: zero,
          oth: { igst: 0, cgst: 5000, sgst: 5000, cess: 0 },
          blocked: zero,
        },
