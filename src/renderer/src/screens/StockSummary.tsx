...
type StockTab = 'summary' | 'serials' | 'costing' | 'labels'
 ...
 *
/**
 * Tabs rather than four more sidebar entries: every letter A-Z is already an accelerator (see
 * `__tests__/accel.test.ts`), and more to the point these are all the same subject — a person
 * printing shelf labels or looking up a serial is doing stock work and expects to find it where
 * stock is. The tab is remembered across sessions, which is roadmap A #10's rule applied here.
 *
 * Job work is NOT one of these tabs, though this lane built one. It has its own screen
 * (`screens/JobWork.tsx`) from the ITC-04 lane, with the section 143 clock and the return on it,
 * and two places to record the same challan is how the paperwork and the stock come to disagree.
 */
