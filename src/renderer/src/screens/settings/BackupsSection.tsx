    ...
    <div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-2">
        ...
        <Panel className="px-4 py-4">
          ...
          {rotation.data && <div className="mt-4 border-t border-line pt-3"><p className="text-[9px] font-semibold uppercase tracking-[0.12em] text-muted">Rotation forecast</p><p className="mt-1 text-[10px]">Up to {rotation.data.forecast.projectedRetainedFiles} managed restore points · about {formatSize(rotation.data.forecast.projectedBytes)}</p><p className="mt-1 text-[8.5px] text-muted">{rotation.data.policy.dailyCount} daily · {rotation.data.policy.weeklyCount} weekly · {rotation.data.policy.monthlyCount} monthly · {rotation.data.policy.yearEndCount} year-end. Manual and pre-change safety points are never removed by rotation.</p>{isOwner && <div className="mt-2 flex gap-1">{[{ label: 'Lean', value: { dailyCount: 7, weeklyCount: 4, monthlyCount: 6, yearEndCount: 5 } }, { label: 'Balanced', value: { dailyCount: 14, weeklyCount: 8, monthlyCount: 12, yearEndCount: 7 } }, { label: 'Deep', value: { dailyCount: 30, weeklyCount: 12, monthlyCount: 24, yearEndCount: 10 } }].map((preset) => <Button key={preset.label} variant="ghost" className="!min-h-0 !px-2 !py-1" onClick={async () => { try { await api.backups.setRotation(preset.value); await queryClient.invalidateQueries({ queryKey: ['backup-rotation'] }); toast.push('success', `${preset.label} rotation will apply to automatic and year-end backups`) } catch (error) { toast.push('error', (error as Error).message) } }}>{preset.label}</Button>)}</div>}</div>}
        </Panel>
      </div>
      {restoring && (
        <RestoreModal
