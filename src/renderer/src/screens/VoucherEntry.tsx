...
export function VoucherEntry({
  ...
  } = useQuery({ queryKey: ['voucherTypes'], queryFn: api.voucherTypes.list })
  const {
    data: existing,
  ...
  const {
    data: workDraft,
    isError: workDraftError,
    refetch: refetchWorkDraft,
  } = useQuery({
  ...
  const showFirstVoucherHint =
    !voucherId && !workDraftId && !hintDismissed && dash?.voucherCount === 0
  ...
  const loadFailed =
    typesError ||
    (!!voucherId && existingError) ||
    (!!workDraftId && workDraftError)
  if (loadFailed) {
    return (
      <div className="mx-auto max-w-4xl pt-3">
        <QueryErrorState
          title="Voucher entry could not be opened"
          detail="The local record could not be read. Nothing was changed."
          onRetry={() => {
            void refetchTypes()
            if (voucherId) void refetchExisting()
            if (workDraftId) void refetchWorkDraft()
          }}
        />
      </div>
    )
  }
  if (!types || (voucherId && !existing) || (workDraftId && !workDraft))
  ...
  const availableTypes = types.filter(
    (type) =>
      features.inventory ||
      (type.kind !== 'stock_journal' && type.kind !== 'physical_stock'),
  )
  const primaryTypes = availableTypes.filter((type) =>
    PRIMARY_VOUCHER_KINDS.has(type.kind),
  )
  const additionalTypes = availableTypes.filter(
    (type) => !PRIMARY_VOUCHER_KINDS.has(type.kind),
  )
    ...
    <div className="mx-auto max-w-4xl">
      {showFirstVoucherHint && (
        <div className="mb-4 flex items-center justify-between gap-4 rounded-md border border-amber/40 bg-amber/10 px-4 py-2.5">
          <p className="text-[12.5px] text-ink">
            First voucher? Choose a type or press <Kbd>F8</Kbd> for Sales, fill
            in the lines, then press <Kbd>⌘↵</Kbd> to save.
          </p>
          <button
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <h2 className="mr-auto text-[20px] font-semibold tracking-[-0.015em] whitespace-nowrap">
          {voucherId
            ? `Alter voucher ${existing?.number}`
            : workDraft
              ? `Resume ${workDraft.title}`
              : 'Voucher entry'}
        </h2>
        {voucherId && existing && (
          <Button
            data-testid="btn-duplicate-voucher"
            className="ml-auto"
            disabled={!!existing.reversalOfId || !!existing.reversedById}
            onClick={() => {
              const partyLine = existing.lines.find(
                (line) => line.ledgerId === existing.partyLedgerId,
              )
              const accountLedgerId = existing.lines
                .filter(
                  (line) =>
                    line.ledgerId !== existing.partyLedgerId &&
                    (!partyLine || line.drCr !== partyLine.drCr),
                )
                .sort((a, b) => b.amount - a.amount)[0]?.ledgerId
              nav.go({
                name: 'voucher-entry',
                kindHint: currentType.kind,
                draftId: nextDraftId(),
                draft: {
                  date: todayISO(),
                  partyLedgerId: existing.partyLedgerId,
                  ...
                  accountLedgerId,
                  inventory: existing.inventory.map(
                    ({
                      stockItemId,
                      godownId,
                      batchId,
                  ...
                  posOverride: existing.posOverride,
        ...
        {voucherId &&
          ...
          !existing.reversedById && (
            <Button
              ...
              disabled={
                !!existing.tds ||
                existing.inventory.some((line) => line.isAbsolute)
              }
              disabledTitle={
                existing.tds
                  ? 'Use a TDS adjustment for this voucher'
                  : 'Use a new physical stock count or stock adjustment'
              }
        ...
        {voucherId && existing && (
          <Button
            data-testid="btn-voucher-comments"
            variant="ghost"
        ...
        {voucherId && existing && (
          <Button
            data-testid="btn-voucher-attachments"
            variant="ghost"
        ...
        {!voucherId && !workDraftId && (
          <Button
            data-testid="btn-compound-entry"
            variant="ghost"
      ...
      {!voucherId && (
        <div
          className="mb-4 flex min-w-0 items-center gap-1 overflow-visible border-y border-line bg-panel px-1 py-1"
          role="toolbar"
            ...
            <button
              ...
              className={`flex min-h-8 shrink-0 items-center gap-1.5 rounded-md px-2.5 py-1 text-[12px] transition-colors ${
                type.id === currentType.id
                  ? 'bg-amber/20 font-medium text-amber'
          ...
          {additionalTypes.length > 0 && (
            <details className="relative ml-auto shrink-0">
              <summary className="flex min-h-8 cursor-pointer list-none items-center rounded-md px-2.5 py-1 text-[12px] text-muted hover:bg-panel2 hover:text-ink">
                {additionalTypes.some((type) => type.id === currentType.id)
                  ? currentType.name
                  : 'More types'}
              </summary>
              <div className="absolute right-0 z-20 mt-1 min-w-44 overflow-hidden rounded-lg border border-line bg-panel py-1 panel-shadow">
                {additionalTypes.map((type) => (
                  <button
                    ...
                    data-testid={`tab-voucher-entry-${type.kind}`}
                    aria-current={
                      type.id === currentType.id ? 'page' : undefined
                    }
                    aria-pressed={type.id === currentType.id}
                    onClick={(event) => {
                      setTypeId(type.id)
      ...
      {existing?.reversalOfId && (
        <div className="mb-4 flex items-center justify-between rounded-md border border-cr/25 bg-cr/5 px-3 py-2 text-[11.5px]">
          <span>
            This is a linked reversal by{' '}
            {existing.reversalAuthor ?? 'Local user'} ·{' '}
          ...
          <Button
            ...
            onClick={() =>
              nav.go({
                name: 'voucher-entry',
                voucherId: existing.reversalOfId!,
              })
            }
          >
            Open original
        ...
        <div className="mb-4 flex items-center justify-between rounded-md border border-line bg-panel2 px-3 py-2 text-[11.5px]">
          ...
          <Button
            ...
            onClick={() =>
              nav.go({
                name: 'voucher-entry',
                voucherId: existing.reversedById!,
              })
            }
          >
            Open reversal
      ...
      <p className="mt-3 text-[11.5px] text-muted">
        <Kbd>F4</Kbd>-<Kbd>F9</Kbd> switch type · <Kbd>⌘↵</Kbd> save ·{' '}
        <Kbd>Esc</Kbd> back · dates accept <span className="num">7</span>,{' '}
        <span className="num">7/4</span>, <span className="num">y</span>
      </p>
      {reverseOpen && voucherId && (
        <VoucherReverseModal
          ...
          onReverse={async (date, reason) => {
            try {
              const [reversal] = await api.vouchers.batchReverse(
                [voucherId],
                date,
                reason,
              )
              ...
              if (reversal)
                nav.go({ name: 'voucher-entry', voucherId: reversal.id })
            } catch (err) {
              toast.push(
                'error',
      ...
      {commentsOpen && voucherId && (
        <VoucherCommentsModal
          voucherId={voucherId}
          onClose={() => setCommentsOpen(false)}
        />
      )}
      {attachmentsOpen && voucherId && (
      ...
      {compoundOpen && (
        <CompoundEntryModal onClose={() => setCompoundOpen(false)} />
      )}
    </div>
  )
}

function VoucherAttachmentsModal({
  voucherId,
  onClose,
}: {
  voucherId: number
  ...
  const [kind, setKind] = useState<
    'invoice' | 'receipt' | 'email' | 'delivery' | 'other'
  >('invoice')
  const [adding, setAdding] = useState(false)
  const { data, isLoading } = useQuery({
    queryKey: ['voucher-attachments', voucherId],
    queryFn: () => api.vouchers.attachments(voucherId),
  })
  const add = async (): Promise<void> => {
    ...
    try {
      ...
      if (added.length) {
        ...
        toast.push(
          'success',
          `${added.length} evidence file${added.length === 1 ? '' : 's'} attached`,
        )
      }
    } catch (error) {
  ...
  return (
    <Modal title="Voucher evidence bundle" onClose={onClose}>
      <p className="mb-3 text-[12px] leading-5 text-muted">
        Keep invoices, receipts, emails and delivery evidence together. Files
        are copied into this company’s managed attachment vault.
      </p>
      <div
        data-testid="voucher-attachment-list"
        ...
        {isLoading ? (
          ...
          data.map((item) => (
            <button
              key={item.id}
              type="button"
              onClick={() => void api.vouchers.openAttachment(item.id)}
              className="flex w-full items-center justify-between border-b border-line px-3 py-2.5 text-left last:border-0 hover:bg-panel2"
            >
              <span>
      ...
      <div className="mt-4 flex items-end gap-2">
        <Field label="Evidence type">
          <Select
            data-testid="select-voucher-attachment-kind"
            value={kind}
          ...
          >
            ...
            <option value="delivery">Delivery document</option>
            <option value="other">Other</option>
          </Select>
        </Field>
        <Button
          data-testid="btn-add-voucher-attachment"
          variant="primary"
          disabled={adding}
          onClick={() => void add()}
        >
...
const COMPOUND_PRESETS = {
  asset_purchase: {
    ...
    kind: 'journal' as VoucherKind,
    main: 'Asset ledger',
    extra: 'Input tax / expense',
    counter: 'Bank or payable',
  },
  loan_repayment: {
    label: 'Loan repayment',
    kind: 'payment' as VoucherKind,
    main: 'Loan principal',
  ...
  import_purchase: {
    ...
    kind: 'purchase' as VoucherKind,
    main: 'Import purchases',
    extra: 'Duty / landed cost',
    counter: 'Supplier payable',
  },
  advance_adjustment: {
    label: 'Advance adjustment',
    kind: 'journal' as VoucherKind,
    main: 'Customer advance',
    extra: 'Adjustment / tax',
    counter: 'Revenue ledger',
  },
}

function CompoundEntryModal({
  onClose,
}: {
  onClose: () => void
}): React.JSX.Element {
  ...
  const [presetKey, setPresetKey] =
    useState<keyof typeof COMPOUND_PRESETS>('asset_purchase')
  ...
  const build = (): void => {
    ...
    if (
      ...
      !counterLedgerId ||
      !mainAmount ||
      (extra > 0 && !extraLedgerId)
    )
      return void toast.push(
        'warning',
        'Choose the required ledgers and enter the amounts',
      )
    const lines = [
      {
        ...
        : []),
      {
        ...
        drCr: 'cr' as const,
        amount: mainAmount + extra,
        costAllocations: [],
      },
    ]
    nav.go({
      name: 'voucher-entry',
      kindHint: preset.kind,
      draftId: nextDraftId(),
      draft: { date: todayISO(), narration: preset.label, lines },
    })
    onClose()
  }
  const ledgerOptions = (
  ...
  return (
    <Modal title="Compound entry assistant" onClose={onClose}>
      <p className="mb-4 text-[12px] leading-5 text-muted">
        Build a balanced, editable starting entry. Nothing posts until the
        normal validation and approval flow succeeds.
      </p>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Workflow">
          <Select
            data-testid="select-compound-preset"
            value={presetKey}
            onChange={(event) =>
              setPresetKey(event.target.value as keyof typeof COMPOUND_PRESETS)
            }
          >
            {Object.entries(COMPOUND_PRESETS).map(([key, value]) => (
        ...
        <div />
        <Field label={preset.main}>
          <Select
            value={mainLedgerId}
            onChange={(event) => setMainLedgerId(Number(event.target.value))}
          >
            {ledgerOptions}
          </Select>
        </Field>
        <Field label="Principal / base amount">
          <AmountInput paise={mainAmount} onPaise={setMainAmount} />
        </Field>
        <Field label={preset.extra}>
          <Select
            value={extraLedgerId}
            onChange={(event) => setExtraLedgerId(Number(event.target.value))}
          >
            {ledgerOptions}
          </Select>
        </Field>
        <Field label="Additional amount (optional)">
          <AmountInput paise={extraAmount} onPaise={setExtraAmount} />
        </Field>
        <Field label={preset.counter}>
          <Select
            value={counterLedgerId}
            onChange={(event) => setCounterLedgerId(Number(event.target.value))}
          >
            {ledgerOptions}
          </Select>
        </Field>
        <div className="self-end rounded-md border border-line bg-panel2 px-3 py-2 text-[11px] text-muted">
          Credit total:{' '}
          <span className="num text-ink">
            ₹{(((mainAmount ?? 0) + (extraAmount ?? 0)) / 100).toFixed(2)}
          </span>
        </div>
      </div>
      <div className="mt-5 flex justify-end gap-2">
        ...
        <Button
          data-testid="btn-build-compound-entry"
          variant="primary"
          onClick={build}
        >
...
function VoucherCommentsModal({
  ...
  return (
    <Modal title="Voucher comments" onClose={onClose}>
      <p className="mb-3 text-[12px] leading-5 text-muted">
        Review notes stay separate from the printed narration. Existing comments
        cannot be rewritten.
      </p>
      <div
        data-testid="voucher-comment-list"
        className="max-h-64 overflow-y-auto rounded-md border border-line"
      >
        {isLoading ? (
          <div className="p-3">
          ...
          data.map((comment) => (
            ...
            >
              <p className="whitespace-pre-wrap text-[12.5px] leading-5 text-ink">
                {comment.body}
              </p>
      ...
      <label className="mt-4 block">
        ...
        <textarea
          data-testid="input-voucher-comment"
          value={body}
          onChange={(event) => setBody(event.target.value)}
          maxLength={2000}
          rows={3}
          placeholder="Question, evidence needed, or review context"
          className="w-full resize-none rounded-md border border-line bg-panel2 px-3 py-2 text-[12.5px] leading-5 text-ink placeholder:text-muted/65"
        />
      </label>
      <div className="mt-3 flex items-center justify-between">
        <span className="num text-[10px] text-muted">{body.length}/2,000</span>
        <div className="flex gap-2">
          <Button onClick={onClose}>Close</Button>
          <Button
            data-testid="btn-add-voucher-comment"
            variant="primary"
