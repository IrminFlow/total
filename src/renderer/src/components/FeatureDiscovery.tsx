...
export function FeatureDiscovery({
  ...
  if (!readProductFlags(localStorage).flags.featureDiscovery || !tip)
    return null;
  return (
    <aside
      data-testid="feature-discovery"
      aria-label="Feature tip"
      className="mb-3 flex min-h-8 items-center gap-2 border-b border-line/80 px-1 py-1"
    >
      <Lightbulb size={14} weight="fill" className="shrink-0 text-amber" />
      <p
        className="min-w-0 flex-1 truncate text-small text-ink"
      ...
      <button
        className="shrink-0 whitespace-nowrap px-1.5 py-1 text-small text-muted hover:text-ink"
        onClick={() => {
