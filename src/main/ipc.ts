...
export function registerIpc(): void {
  ...
  // ---------- opt-in AI copilot (keys remain in OS-backed safeStorage) ----------
  handle("ai:getConfig", () => ai.getConfig(), "viewer");
  handle(
    "ai:setConfig",
    (p) => ai.setConfig(aiProviderInputSchema.parse(p)),
    "owner",
  );
  ...
  handle(
    "ai:contextPreview",
    (p) => {
      const { from, to, fields } = periodSchema
        .extend({
          fields: z
            .array(
              z.enum([
        ...
        .parse(p);
      const c = requireCompany();
      return ai.contextPreview(c.db, c.info, from, to, fields);
    },
    "viewer",
  );
  handle(
    "ai:ask",
    async (p) => {
      const input = aiAskSchema.parse(p);
      const c = requireCompany();
  ...
  handle(
    "ai:draftVoucher",
    async (p) => {
      const { prompt } = aiDraftVoucherSchema.parse(p);
      const c = requireCompany();
      return ai.draftVoucher(c.db, c.slug, prompt);
    },
    "accountant",
  );
  handle(
    "ai:documents:list",
    () => assistiveAutomation.listDocumentInbox(requireCompany().db),
    "viewer",
  );
  handle("ai:documents:capture", async (p) => {
    const { kind } = z
      .object({ kind: z.enum(["supplier_invoice", "receipt"]) })
      .parse(p);
    const picked = await dialog.showOpenDialog({
      title:
        kind === "supplier_invoice"
          ? "Choose a supplier invoice image"
    ...
    const company = requireCompany();
    const db = company.db;
    const route = assistiveAutomation
      .listTaskRoutes(db)
      .find((row) => row.taskKind === "ocr");
    const extracted = await ai.extractDocumentImage(
      picked.filePaths[0],
      kind,
      route?.model,
      route?.provider,
    ...
    const storedPath = join(
      documentDir,
      `${kind}-${randomUUID()}${extname(picked.filePaths[0]).toLowerCase()}`,
    );
    const managedPath = attachmentVault.storeManagedAttachment(
      db,
      company.slug,
      picked.filePaths[0],
      storedPath,
    );
    return assistiveAutomation.addExtractedDocument(
      db,
      kind,
      managedPath,
      extracted,
  ...
  handle("ai:documents:review", (p) => {
    const { id, status } = z
      ...
      .parse(p);
    return assistiveAutomation.reviewDocument(
      requireCompany().db,
      id,
      status,
      sessionUser?.name ?? "Local user",
  ...
  handle(
    "ai:ledgerSuggestions",
    (p) => {
      const { kind, query, contextKey, partyLedgerId } = z
        .object({
        ...
        .parse(p);
      return assistiveAutomation.evidenceLedgerSuggestions(
        requireCompany().db,
        kind,
        query,
        contextKey,
  ...
  handle("ai:ledgerFeedback", (p) => {
    const { contextKey, ledgerId, outcome } = z
      .object({
        contextKey: z.string().trim().min(1).max(200),
        ledgerId: z.number().int().positive(),
      ...
      .parse(p);
    assistiveAutomation.recordLedgerFeedback(
      requireCompany().db,
      contextKey,
      ledgerId,
      outcome,
  ...
  handle(
    ...
    (p) => {
      const { query } = z
        ...
        .parse(p);
      return assistiveAutomation.constrainedNaturalSearch(
        requireCompany().db,
        query,
      );
    },
  ...
  handle(
    ...
    (p) => {
      const input = z
        ...
        .parse(p);
      return assistiveAutomation.reconciliationExplanation(
        input.kind,
  ...
  handle(
    "ai:collectionMessage",
    (p) => {
      const { ledgerId, asOn, tone, billVoucherIds } = z
        .object({
          ledgerId: z.number().int().positive(),
          asOn: isoDate,
        ...
        .parse(p);
      return assistiveAutomation.collectionMessage(
        requireCompany().db,
        ledgerId,
        asOn,
        tone,
  ...
  handle(
    "ai:routes:list",
    () => assistiveAutomation.listTaskRoutes(requireCompany().db),
    "viewer",
  );
  handle(
    "ai:routes:set",
    (p) => {
      ...
      return assistiveAutomation.setTaskRoute(
        requireCompany().db,
        input,
        sessionUser?.name ?? "Local user",
      );
  ...
  handle(
    ...
    (p) => {
      const input = z
        ...
        .parse(p);
      return {
        id: assistiveAutomation.recordEvaluation(requireCompany().db, input),
      };
    },
    "owner",
  );
