 ...
 *
 * The table is `stock_item_gst_rates`, created by migration 40 and already on real databases. This
 * lane originally created its own `item_gst_rates` with the same columns under different names;
 * that table was dropped in the merge rather than shipped alongside, because two tables holding
 * the same fact is two answers to one question. `gst_rate`/`cess_rate` are aliased to
 * `ratePercent`/`cessPercent` on the way out, so the engine's shape is unchanged.
 */
...
const SELECT = `SELECT id, stock_item_id AS stockItemId, effective_from AS effectiveFrom,
                       gst_rate AS ratePercent, cess_rate AS cessPercent, note
                FROM stock_item_gst_rates`
...
export function saveItemRate(db: DB, input: ItemRateInput, id?: number): SavedItemRate {
  ...
  if (id != null) {
    db.prepare(
      `UPDATE stock_item_gst_rates SET effective_from = ?, gst_rate = ?, cess_rate = ?, note = ? WHERE id = ?`
...
export function makeRateResolver(db: DB): ((stockItemId: number, date: string) => RateChange | null) | null {
  const { n } = db.prepare('SELECT COUNT(*) AS n FROM stock_item_gst_rates').get() as { n: number }
  if (n === 0) return null
