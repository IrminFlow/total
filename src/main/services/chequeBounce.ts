...
import { deleteVoucher, getVoucher, NOT_DELETED, saveVoucher } from './vouchers'
  ...
  return db
    .prepare(
      `SELECT b.id, b.voucher_id AS voucherId, v.number AS voucherNumber, v.date AS voucherDate,
       ...
       -- NOT_DELETED, not IN_BOOKS. A bounce whose receipt has been binned does not belong in a
       -- register of what happened: binning the receipt is the assertion that it never did. But
       -- IN_BOOKS also excludes post-dated vouchers, and a post-dated cheque receipt is post-dated
       -- by definition — using it here would empty the register of exactly the cheques it lists.
       -- Written inline rather than folded into the optional clause, so the guard in
       -- notDeleted.test.ts can see it: a scope assembled at runtime is one no test can check.
 ...
 *
 * Binned receipts are excluded, and that is the point: a receipt entered by mistake, marked
 * bounced and then binned would otherwise count against the customer forever, on the strength of
 * an entry the books say never happened.
 */
  ...
  return db
    .prepare(
      `SELECT v.party_ledger_id AS partyLedgerId, p.name AS partyName, COUNT(*) AS bounces
       ...
       WHERE v.party_ledger_id IS NOT NULL AND ${NOT_DELETED}
