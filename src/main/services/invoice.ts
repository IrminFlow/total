...
import { invoiceLabels } from "@shared/invoiceLabels";
 ...
 */
export function hsnSummaryForInvoice(
  inv: EdocInvoice,
  supplyHint?: "inter" | "intra",
): HsnSummaryRow[] {
  const supply: "inter" | "intra" =
    supplyHint ?? (inv.igst > 0 ? "inter" : "intra");
  ...
  return [...buckets.values()]
    .sort((a, b) =>
      a.hsn === b.hsn ? a.rate - b.rate : a.hsn < b.hsn ? -1 : 1,
    )
    .map((b) => {
...
export function buildInvoiceHtml(
  ...
  const isIntra =
    inv.igst > 0
      ...
      : inv.cgst > 0 || inv.sgst > 0
        ? true
        : (inv.supTyp == null || inv.supTyp === "B2B") &&
          inv.pos === company.stateCode;
  const showHsn = config.showHsn;
  const showDiscount = config.showDiscount;
  const showBarcode =
    config.showItemBarcode && inv.items.some((i) => i.barcode);
  const labels = invoiceLabels(config.labelLanguage);
  const columnCount =
    6 + (showHsn ? 1 : 0) + (showBarcode ? 1 : 0) + (showDiscount ? 1 : 0);
  ...
  const headRow = `<tr>
          <th scope="col" class="c" style="width:34px">#</th><th scope="col">${esc(labels.description)}</th>
          ${showHsn ? '<th scope="col" class="c" style="width:80px">HSN</th>' : ""}
          ${showBarcode ? '<th scope="col" class="c" style="width:100px">Barcode</th>' : ""}
          <th scope="col" class="r" style="width:90px">${esc(labels.quantity)}</th><th scope="col" class="r" style="width:100px">${esc(labels.rate)}</th>
          ${showDiscount ? `<th scope="col" class="r" style="width:80px">${esc(labels.discount)}</th>` : ""}
          <th scope="col" class="c" style="width:60px">GST</th><th scope="col" class="r" style="width:110px">${esc(labels.amount)}</th>
        </tr>`;
  ...
  if (inv.items.length <= INVOICE_ITEMS_PER_PAGE) {
    itemsBlock = `
      <table class="items" aria-label="${esc(labels.invoice)} ${esc(inv.number)}">
        <thead>${headRow}</thead>
    ...
    for (
      let start = 0;
      start < inv.items.length;
      start += INVOICE_ITEMS_PER_PAGE
    ) {
      ...
      const broughtForward =
        start > 0
          ? `<tr class="cf"><td colspan="${columnCount - 1}" class="r">${esc(labels.broughtForward)}</td><td class="r num">${money(cumulative)}</td></tr>`
          : "";
      cumulative += slice.reduce((s, it) => s + it.taxablePaise, 0);
      const carriedForward = !isLast
        ? `<tr class="cf"><td colspan="${columnCount - 1}" class="r">${esc(labels.carriedForward)}</td><td class="r num">${money(cumulative)}</td></tr>`
        : "";
      chunks.push(`
      <table class="items${isLast ? "" : " page-split"}" aria-label="${esc(labels.invoice)} ${esc(inv.number)}">
        <thead>${headRow}</thead>
  ...
  const hsnRows = showHsn
    ? hsnSummaryForInvoice(inv, isIntra ? "intra" : "inter")
    : [];
  const anyCess = hsnRows.some((r) => r.cess > 0);
  const hsnBlock = hsnRows.length
    ? `
      <table class="hsn" aria-label="HSN tax summary">
        <thead><tr>
          <th scope="col">HSN/SAC</th><th scope="col" class="c" style="width:60px">${esc(labels.rate)}</th><th scope="col" class="r" style="width:110px">${esc(labels.taxable)}</th>
  ...
  const taxRows = [
    isIntra
      ? `<tr><td>CGST</td><td class="r num">${money(inv.cgst)}</td></tr>`
      : "",
    isIntra
      ? `<tr><td>SGST</td><td class="r num">${money(inv.sgst)}</td></tr>`
      : "",
    !isIntra
      ? `<tr><td>IGST</td><td class="r num">${money(inv.igst)}</td></tr>`
      : "",
    inv.cess > 0
      ? `<tr><td>Cess</td><td class="r num">${money(inv.cess)}</td></tr>`
      : "",
    inv.roundOff !== 0
      ? `<tr><td>${esc(labels.roundOff)}</td><td class="r num">${money(inv.roundOff)}</td></tr>`
      : "",
  ].join("");

  const logoBlock = config.logoDataUrl
    ? `<img src="${esc(config.logoDataUrl)}" alt="${esc(company.name)} logo" style="max-height:60px;max-width:220px;object-fit:contain;margin-bottom:6px" />`
    : "";

  const bankBlock = config.bankDetails
    ? `<div style="margin-top:10px" class="lbl">${esc(labels.bankDetails)}</div>
  ...
  const qrBlock = config.showQr
    ? `<div style="margin-top:8px" role="img" aria-label="${esc(labels.verificationQr)}">
         ${qrSvg(
           einvoiceQrPayload({
         ...
         <div style="font-size:8.5px;color:#555;text-align:center;margin-top:2px">${esc(labels.verificationQr)}</div>
       </div>`
    : "";

  const sheet = `
    <main class="sheet" aria-label="${esc(labels.invoice)} ${esc(inv.number)}">
      <div class="head">
        <div>
          ${logoBlock}
          <h1>${esc(company.name)}</h1>
          <div>${esc(company.address)}</div>
          <div class="num">GSTIN: ${esc(company.gstin ?? labels.unregistered)} | ${esc(GST_STATES[company.stateCode] ?? company.stateCode)}</div>
        </div>
        <div class="tag">
      ...
      <div class="meta">
        <div>
          <div class="lbl">${esc(labels.billedTo)}</div>
          <div><b>${esc(inv.partyName ?? labels.cashSale)}</b></div>
          <div>${esc(inv.partyAddress)}</div>
          <div class="num">${inv.partyGstin ? "GSTIN: " + esc(inv.partyGstin) : esc(labels.unregistered)}</div>
        </div>
        <div>
          <div class="lbl">${esc(labels.invoice)}</div>
          <div>${esc(labels.number)}: <b class="num">${esc(inv.number)}</b></div>
          <div>${esc(labels.date)}: <span class="num">${toDisplayDate(inv.date)}</span></div>
          <div>${esc(labels.placeOfSupply)}: <span class="num">${esc(inv.pos)}-${esc(GST_STATES[inv.pos] ?? "")}</span></div>
          ${inv.vehicleNo ? `<div>${esc(labels.vehicle)}: <span class="num">${esc(inv.vehicleNo)}</span></div>` : ""}
        </div>
      </div>
      ${itemsBlock}
      ${hsnBlock}
      <div class="bottom">
        <div class="words">
          <div class="lbl">${esc(labels.amountInWords)}</div>
        ...
        <table class="tot" aria-label="${esc(labels.total)}">
          <tr><td>${esc(labels.taxableValue)}</td><td class="r num">${money(inv.taxable)}</td></tr>
          ${taxRows}
          <tr class="grand"><td>${esc(labels.total)}</td><td class="r num">₹ ${money(inv.total)}</td></tr>
        </table>
      </div>
      <div class="sig">
        <div>${esc(labels.receiverSignature)}</div>
        <div class="for">${esc(labels.forCompany)} <b>${esc(company.name)}</b><br/><br/><br/>${esc(config.signatory)}</div>
      </div>
      ${
        config.showEnteredBy && audit && (audit.enteredBy || audit.alteredBy)
          ? `<div class="audit-foot">${[
              audit.enteredBy
                ? `${esc(labels.enteredBy)} ${esc(audit.enteredBy)}`
                : "",
              audit.alteredBy
                ? `${esc(labels.alteredBy)} ${esc(audit.alteredBy)}`
                : "",
            ]
              .filter(Boolean)
              .join(" | ")}</div>`
  ...
  const copies = config.copyLabels
    .map(
      (label) => `
      <section class="copy" aria-label="${esc(label)}">
...
function attachDiscounts(db: DB, voucherId: number, inv: EdocInvoice): void {
  const rows = db
    .prepare(
      "SELECT discount_paise AS d FROM inventory_lines WHERE voucher_id = ? ORDER BY line_order, id",
    )
...
function auditTrailFor(db: DB, voucherId: number): InvoiceAuditTrail {
  const entered = db
    .prepare(
      "SELECT user_name AS u FROM audit_log WHERE entity = 'voucher' AND entity_id = ? AND action = 'create' ORDER BY id LIMIT 1",
    )
    .get(voucherId) as { u: string | null } | undefined;
  const altered = db
    .prepare(
      "SELECT user_name AS u FROM audit_log WHERE entity = 'voucher' AND entity_id = ? AND action = 'update' ORDER BY id DESC LIMIT 1",
    )
    .get(voucherId) as { u: string | null } | undefined;
...
export function invoiceHtml(
  ...
  if (!inv)
    throw new Error("Invoice not found (only sales vouchers can be printed)");
  attachDiscounts(db, voucherId, inv);
  const config = getInvoiceConfig(db);
  return {
    html: buildInvoiceHtml(company, config, inv, auditTrailFor(db, voucherId)),
    number: inv.number,
  };
}
...
export function invoicePreviewHtml(
  ...
  const config = configOverride
    ? mergeInvoiceConfig({ ...saved, ...configOverride })
    : saved;
  if (voucherId != null) {
    const [inv] = extractEdocInvoices(
      db,
...
export async function invoicePdf(
  ...
  return writeExportPdf(slug, `invoice-${safe}.pdf`, html, {
    pageSize: "A4",
    pageNumbers: true,
  });
}
...
export async function invoicePdfBatch(
  ...
  for (const voucherId of voucherIds) {
    ...
    try {
      ...
      throw new Error(
        `Voucher #${voucherId}: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
