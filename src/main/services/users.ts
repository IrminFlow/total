...
export function listUsers(db: DB): User[] {
  const rows = db
    .prepare(
      "SELECT id, name, role, active, created_at AS createdAt, access_expires_at AS accessExpiresAt FROM users ORDER BY name COLLATE NOCASE",
    )
    .all() as UserRow[];
...
export function listLoginNames(
  db: DB,
): { id: number; name: string; role: Role }[] {
  return db
    .prepare(
      "SELECT id, name, role FROM users WHERE active = 1 AND (access_expires_at IS NULL OR access_expires_at > datetime('now')) ORDER BY name COLLATE NOCASE",
    )
...
export function getUser(db: DB, id: number): User {
  const row = db
    .prepare(
      "SELECT id, name, role, active, created_at AS createdAt, access_expires_at AS accessExpiresAt FROM users WHERE id = ?",
    )
...
export function saveUser(db: DB, input: UserInput, id?: number): User {
  if (id === undefined) {
    ...
    const result = db
      .prepare(
        "INSERT INTO users (name, pin_hash, role, active, access_expires_at) VALUES (?, ?, ?, ?, ?)",
      )
      .run(
        input.name,
        pinHash,
  ...
  if (input.pin) {
    const pinHash = hashPin(input.pin);
    db.prepare(
      "UPDATE users SET name = ?, role = ?, active = ?, pin_hash = ?, access_expires_at = ? WHERE id = ?",
    ).run(
      input.name,
      input.role,
      input.active === false ? 0 : 1,
    ...
    db.prepare(
      "UPDATE users SET name = ?, role = ?, active = ?, access_expires_at = ? WHERE id = ?",
    ).run(
      input.name,
      input.role,
      input.active === false ? 0 : 1,
...
function readThrottle(db: DB, userId: number): ThrottleState | null {
  const row = db
    ...
    .get(throttleKey(userId)) as { value: string } | undefined;
  if (!row) return null;
  try {
...
function writeThrottle(db: DB, userId: number, state: ThrottleState): void {
  ...
  ).run(throttleKey(userId), JSON.stringify(state));
}
...
export function login(db: DB, userId: number, pin: string): LoginResult {
  ...
  const row = db
    .prepare(
      "SELECT id, name, pin_hash AS pinHash, role, active, access_expires_at AS accessExpiresAt FROM users WHERE id = ?",
    )
    .get(userId) as
    | {
        id: number;
  ...
  const unexpired =
    ...
    new Date(row.accessExpiresAt).getTime() > now;
  const ok =
    row && row.active && unexpired ? verifyPin(pin, row.pinHash) : false;
  ...
  writeAudit(db, "user", row.id, "login", null, {
    name: row.name,
    role: row.role,
  });
