...
export function registerMigrationHandlers({
  ...
  handle("import:profileApply", async (payload) => {
    ...
    const company = requireCompany();
    const profile = migrationTools
      .listMappingProfiles(company.db)
      .find((row) => row.id === profileId);
    if (!profile) throw new Error("Mapping profile not found");
  ...
  handle(
    ...
    async (payload) => {
      const { batchId } = z
        .object({ batchId: z.number().int().positive() })
        .strict()
        .parse(payload);
      const company = requireCompany();
      const result = await exportMigrationCertificate(
        company.db,
        company.info,
        company.slug,
        batchId,
      ...
      shell.showItemInFolder(result.jsonPath);
      return result;
    },
    "owner",
  );
  ...
  handle("tally:import", async (payload) => {
    const { xmlText, filePath, dryRun } = tallyImportSchema.parse(
      payload ?? {},
    );
    const company = requireCompany();
    let xml = xmlText;
    ...
    return {
      filePath: resolvedPath ?? null,
      summary: importTallyXml(company.db, xml),
    };
  });
}
