...
await scenario('11-keyboard', async (h) => {
  ...
  const home = async () => {
    // Blur first. Some screens park the caret in a field on arrival (Counter mode's scan box),
    // and from inside a field Escape only means "leave the field" — the `g` after it would be
    // typed into the box rather than navigating, and the sweep would sit there until it timed out.
