...
await scenario('51-inventory-lane', async (h) => {
  ...
  // ---------- #127 job work: the stock half, on the ITC-04 lane's challan ----------
  // This lane built a second job-work service; the merge kept the one with the screen, the
  // section 143 clock and the ITC-04 return on it, and moved the STOCK MOVEMENT onto it. So the
  // channels here are `jobWork:*`, and what is asserted is that saving a challan also moves goods.
  ...
  const challan = await h.invoke('jobWork:save', {
    data: {
      jobWorkerLedgerId: worker.id,
      number: 'LANE-JW-1',
      // After the 15 June purchase, so there is something on the shelf to send.
      date: '2026-06-20',
      goodsType: 'input',
      stockItemId: steel.id,
      description: 'Steel sheet for bending',
      qtyMilli: 5_000
    }
  })
  assert(challan.godownName === 'Job work — Lane Fabrication', 'the goods move to a godown named for the job worker')
  assert(challan.voucherId !== null, 'and a stock journal is what moved them')
  ...
  const held = (await h.invoke('stock:byGodown', { asOn: '2026-06-30' }))
    .filter((r) => r.godownName === 'Job work — Lane Fabrication' && r.stockItemId === steel.id)
  assert(held.length === 1 && held[0].closingQtyMilli === 5_000, 'the stock report says what is lying with the job worker')

  await h.invoke('jobWork:saveReturn', {
    data: { challanId: challan.id, date: '2026-07-01', qtyMilli: 4_500, disposition: 'returned' }
  })
  const withWaste = await h.invoke('jobWork:saveReturn', {
    data: { challanId: challan.id, date: '2026-07-01', qtyMilli: 500, disposition: 'waste_and_scrap' }
  })
  assert(withWaste.balanceMilli === 0, 'goods plus waste closes the challan')

  const emptied = (await h.invoke('stock:byGodown', { asOn: '2026-07-31' }))
    .filter((r) => r.godownName === 'Job work — Lane Fabrication' && r.stockItemId === steel.id)
  assert(emptied.length === 0, 'and the job worker is holding nothing once it is all accounted for')

  const itc04 = await h.invoke('jobWork:itc04', { fyStartYear: 2026 })
  assert(itc04.form !== undefined, 'and the ITC-04 return still builds from the same rows')
