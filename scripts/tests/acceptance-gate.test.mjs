...
const approvers = [{ name: 'Acceptance owner', role: 'Release acceptance', decision: 'approved' }]
...
function validMigration() {
  ...
  return {
    ...
    evidence: {
      ...
      testedAt,
      testedArtifact: { name: 'Total-0.5.0.dmg', bytes: Buffer.byteLength(artifactBody), sha256: digest(artifactBody) },
      approvedAt,
      approvers,
...
function validHuman() {
  return {
    ...
    cohorts: ['bookkeeper', 'business-owner', 'chartered-accountant', 'payroll-operator', 'inventory-manufacturing'].map((id) => ({
      ...
      participants: 1,
      durationMinutes: 60,
      scenarios: ['one', 'two', 'three'].map((name) => ({ name, durationMinutes: 15, evidenceSha256: digest(`${id}-${name}`), result: 'passed' })),
      blockers: [],
    })),
  }
}
...
function validCleanMachine() {
  ...
  return {
    ...
    evidence: {
      ...
      approvers,
      platforms: [
        { id: 'macos-arm64', cleanDevice: true, osVersion: 'macOS 15.6 (24G84)', architecture: 'arm64', installerName: 'Total-0.5.0.dmg', installerBytes: Buffer.byteLength(dmg), installerSha256: digest(dmg), checks },
        { id: 'macos-intel', cleanDevice: true, osVersion: 'macOS 15.6 (24G84)', architecture: 'x64', installerName: 'Total-0.5.0.dmg', installerBytes: Buffer.byteLength(dmg), installerSha256: digest(dmg), upgradeNote: 'No public Intel v0.4 build exists.', checks: { ...checks, upgradeFromV04: 'not_applicable' } },
        { id: 'windows-11', cleanDevice: true, osVersion: 'Windows 11 24H2 build 26100', architecture: 'x64', installerName: 'Total.Setup.0.5.0.exe', installerBytes: Buffer.byteLength(exe), installerSha256: digest(exe), checks },
      ],
    },
  }
}
...
test('treats blocker severity case-insensitively', () => {
  const evidence = validHuman()
  evidence.cohorts[0].blockers.push({ severity: ' P1 ', title: 'Core workflow blocked' })
  const result = runGate(evidence)
  assert.notEqual(result.status, 0)
...
test('production readiness passes the exact release revision into acceptance validation', () => {
  const { evidence } = validMigration()
  const { path } = tempEvidence(evidence)
  const env = { ...process.env, RELEASE_REVISION: revision, MIGRATION_ACCEPTANCE_EVIDENCE: path }
  const output = JSON.parse(execFileSync(process.execPath, [readiness], { cwd: root, env, encoding: 'utf8' }))
  assert.equal(output.checks.find((check) => check.id === 'real-migration-acceptance')?.status, 'ready')

  evidence.testedRevision = 'b'.repeat(40)
  writeFileSync(path, `${JSON.stringify(evidence)}\n`)
  const mismatch = JSON.parse(execFileSync(process.execPath, [readiness], { cwd: root, env, encoding: 'utf8' }))
