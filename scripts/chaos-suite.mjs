...
const targets = [
  ...
  "src/main/services/migrationTools.dbtest.ts",
];
...
const fileResult = spawnSync("npm", ["test", "--", "--run", "src/main/services/exportSigning.test.ts", "src/main/atomicFile.test.ts", "src/shared/boundaryFuzz.test.ts", "src/shared/invoiceConfig.test.ts"], {
  stdio: "inherit",
  env: { ...process.env, TOTAL_CHAOS_SUITE: "1" },
});
if (fileResult.status !== 0) process.exit(fileResult.status ?? 1);

const allTargets = [...targets, "src/main/services/exportSigning.test.ts", "src/main/atomicFile.test.ts", "src/shared/boundaryFuzz.test.ts", "src/shared/invoiceConfig.test.ts"];
