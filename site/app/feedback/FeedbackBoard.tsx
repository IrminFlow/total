...
export default function FeedbackBoard(): React.JSX.Element {
  ...
  const action = async (name: 'vote' | 'follow', ideaId: string): Promise<void> => {
    if (name === 'follow' && !followEmail) return void setError('Enter your email above before following an idea.')
    const response = await fetch('/api/feedback', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: name, ideaId, email: name === 'follow' ? followEmail : '' }) })
  ...
  return <div className="feedback-board">
    <div className="feedback-toolbar"><p>{ideas.length} visible ideas · accounting data is never attached</p><label>Email for follows<input type="email" value={followEmail} onChange={(event) => setFollowEmail(event.target.value)} placeholder="you@business.com" /></label><button className="btn" onClick={() => setCompose((value) => !value)}>{compose ? 'Cancel' : 'Suggest an idea'}</button></div>
