...
127. ✓ Job-work stock sent out and received back (L) — checked first, and there was no ITC-04
     and no job-work service anywhere in the app; D #89 is still open. So this is the STOCK half,
     built with the data shaped as that return's own table (`jobwork:itc04`) so #89 extends it
     rather than growing a second implementation beside it.
